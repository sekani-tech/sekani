    ...
    while ($rx = mysqli_fetch_array($query_client_ln)) {
        ...
        if (mysqli_num_rows($query_loan) > 0) {
            while ($uq = mysqli_fetch_array($query_loan)) {
                $loan_id = $uq["id"];
                // put
                $update_loan = mysqli_query($connection, "UPDATE `loan` SET principal_amount_proposed = '$total', principal_amount = '$total', approved_principal = '$total' WHERE id = '$loan_id'");
                if ($update_loan) {
                    $update_disbursment_cache = mysqli_query($connection, "UPDATE `loan_disbursement_cache` SET principal_amount_proposed = '$total', principal_amount = '$total', approved_principal = '$total' WHERE client_id = '$new_client_id'");
