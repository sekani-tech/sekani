...
if(isset($_POST["paymentgl"]))
{
    ...
    $prod = $_POST['prod_id'];
    $dmsim = "SELECT * FROM ftd_acct WHERE ftd_id = '$prod' AND gl_code = '$paymentgl' && type ='pay'";
    ...
    if($paymentgl == $dfe)
    {
        ...
        $chg = '';
          ...
          $gl_name = $o["name"];
          $gl_name2 = $ox["name"];
          $inload = mysqli_query($connection, "UPDATE `ftd_acct` SET `gl_code` = '{$paymentgl}', `name` = '{$gl_name}', `acct_gl_code` = '{$accountgl}', `acct` = '{$gl_name2}'
    ...
    else
    {
      ...
      $chg = '';
        ...
        $gl_name = $o["name"];
        $gl_name2 = $ox["name"];
        $inload = mysqli_query($connection, "INSERT INTO `ftd_acct` (`int_id`, `gl_code`, `name`, `acct_gl_code`, `acct`, `ftd_id`, `type`)
         VALUES ('{$sint_id}', '{$paymentgl}', '{$gl_name}', '{$accountgl}', '{$gl_name2}', '{$prod}', 'pay')");
       
        // $sql = "SELECT * FROM charge WHERE id = '".$_POST["id"]."'";
        $dis = "Created";
    }
    $sqlx = "SELECT * FROM ftd_acct WHERE ftd_id = '$prod' && type ='pay'";
...
<table id="tabledat" class="table" cellspacing="0" style="width:100%">
  ...
  <?php
    ...
    $prod = $_POST['prod_id'];
    $dmsim = "SELECT * FROM ftd_acct WHERE ftd_id = '$prod' AND gl_code = '$penaltygl' && type ='pen'";
    $dsi = mysqli_query($connection, $dmsim);
    $i = mysqli_fetch_array($dsi);
    $dfe = $i['gl_code'];
    ...
    if($penaltygl == $dfe)
    {
        ...
        $chg = '';
          ...
          $gl_name = $o["name"];
          $gl_name2 = $ox["name"];
          $inload = mysqli_query($connection, "UPDATE `ftd_acct` SET `gl_code` = '{$penaltygl}', `name` = '{$gl_name}', `acct_gl_code` = '{$incomegl}', `acct` = '{$gl_name2}'
    ...
    $sqlx = "SELECT * FROM ftd_acct WHERE ftd_id = '$prod' && type ='pen'";
    $rmes = mysqli_query($connection, $sqlx);
    ?>
<div class="table-responsive">
<table id="tabledat" class="table" cellspacing="0" style="width:100%">
