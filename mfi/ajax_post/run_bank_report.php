...
?>
<?php
if (isset($_POST["start"]) && isset($_POST["bank"]) && isset($_POST["branch"]) && isset($_POST["int_id"]))
{
    $int_id = $_POST["int_id"];
    if($_POST["start"] != '' && $_POST["branch"] != '')
    {
      $std = $_POST["start"];
      $datex= strtotime($std); 
   ...
   $query = mysqli_query($connection, "SELECT * FROM branch WHERE name ='$teller' && int_id='$int_id' && branch_id = '$branch'");
  if (count([$query]) == 1) {
