            ...
            case 'Approval':
                ...
                if ($bulk_options_type === 'Deposit') {
                    foreach ($_POST['checkBoxArray'] as $key => $postValueId) {
                        ...
                        $tellerCon = mysqli_query($connection, "SELECT * FROM `tellers` WHERE int_id = '$transactionCacheInst_id' AND id = '$transactionCacheTellerId' OR name = '$transactionCacheTellerId'");
                        $tellerDetails = mysqli_fetch_array($tellerCon);
                        $tellerNameID = $tellerDetails['id'];
                        $tellerName = $tellerDetails['name'];
                        ...
                        if (!$accountDetails) {
                            // account number not on the accounts table find on group table
                            ...
                            if ($tellerDetails && $accountDetails && $getGlCode && $glAccountDetails && $instDetails) {
                                ...
                                $updateLastDeposit = update('group_balance', $accountId, $accountConstantName, $accountData);
                                //                                insert into account transaction
                                $accountTransDetails = insert('group_transaction', $accountTransData);
                        ...
                        } else if ($transactionCacheAccountNo == $accountDetails['account_no']) {
                            $accountProductId = $accountDetails['product_id'];
                            $accountId = $accountDetails['id'];
                            //                        set all total derived
                            $totalDeposit = $accountDetails['total_deposits_derived'] + $transactionCacheAmount;
                    ...
                    foreach ($_POST['checkBoxArray'] as $key => $postValueId) {
                        ...
                        if (!$accountDetails) {
                            ...
                            $clientCurrentBalance = $accountDetails['account_balance_derived'];
                            ...
                            if ($tellerDetails && $accountDetails && $getGlCode && $glAccountDetails && $instDetails) {
                                //                        update account table
                                $updateLastWithdrawal = update('group_balance', $accountId, $accountConstantName, $accountData);
                                //                                insert into account transaction
                                $accountTransDetails = insert('group_transaction', $accountTransData);
                                //                        update the acc_gl_account
                                if ($transactionCacheIsBank == 0) {
