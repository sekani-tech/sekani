...
if(isset($_POST['downloadPDF'])) {
  ...
  if(isset($_POST['month']) && !empty($_POST['month'])) {
    ...
    if($month == "1") {
        ...
        $current = $mog.$ns;
        $std = $mog.$ms;
        $curdate = "December, ".$mog;
    }
  
  
    function fill_report($connection, $current, $std, $br_id, $branches)
    {
      $out = '';
      $sessint_id = $_SESSION['int_id'];
      //  import
      //  $glcode = $_POST['glcode'];
      $query = "SELECT client.id, client.account_type, client.account_no, client.mobile_no, client.firstname, client.lastname,  staff.first_name, staff.last_name FROM client JOIN staff ON client.loan_officer_id = staff.id WHERE client.int_id = '$sessint_id' && client.status = 'Approved' AND (client.branch_id ='$br_id' $branches) && submittedon_date BETWEEN '$current' AND '$std'";
    ...
    <main>
      <table>
        ...
        <tbody>
          "'.fill_report($connection, $current, $std, $branch_id, $branches).'"
        </tbody>
      </table>
    </main>
    ...
    $file_name = 'registered-client-report-'.$curdate.'.pdf';
    $mpdf->Output($file_name, 'D');
  } 
  else {
    header("Location: ../mfi/report_client_view.php?view5");
  }
