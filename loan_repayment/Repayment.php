...
while ($x = mysqli_fetch_array($select_all_disbursment_cache)) {
    ...
    while ($y = mysqli_fetch_array($select_loan_client)) {
        ...
        if ($dm <= 0 && $int_id != "0") {
            ...
            $d_int_id = $dm["int_id"];
            $collect_loan = mysqli_query($connection, "SELECT * FROM `loan_repayment_schedule` WHERE
                                         ((loan_id = '$d_loan_id' AND client_id = '$d_client_id') 
                                         AND (int_id = '$d_int_id' AND (installment > 0) AND 
                                         (duedate = '$sch_date'))) ORDER BY id ASC LIMIT 1");
            // EXPORT THE IDS HERE
            while ($pop = mysqli_fetch_array($collect_loan)) {
                ...
                $loan_account = mysqli_query($connection, "SELECT * FROM account WHERE account_no = '$acct_no' 
                                                AND client_id = '$collection_client_id' 
                                                AND int_id = '$d_int_id'");
                ...
                if ($client_account_balance >= $collection_due_paid) {
                    ...
                    $update_client_account = mysqli_query($connection, "UPDATE account SET
                                             account_balance_derived = '$balance_remaining', 
                                             total_withdrawals_derived = '$total_withd' 
                                             WHERE int_id = '$int_id' 
                                             AND client_id = '$client_id' 
                ...
                '{$balance_remaining}', '{$gen_date}', '0', '0', '{$collection_due_paid}', '0.00')");
                    ...
                    '0', NULL, '0', '0', '0', '{$total_out_stand}', '{$collection_due_paid}', '{$gen_date}', '0', '{$gen_date}', '0', '1')");
                                ...
                                if ($update_loan_trans) {
                                    ...
                                    $update_repayment = mysqli_query($connection, "INSERT INTO `loan_repayment_schedule_history` (`int_id`, `loan_id`, `client_id`, 
                                                                    `loan_reschedule_request_id`, `fromdate`, `duedate`, 
                                                                    `installment`, `principal_amount`, `interest_amount`, 
                                                                    `fee_charges_amount`, `penalty_charges_amount`, 
                                                                    `createdby_id`, `created_date`, `lastmodified_date`)
                                                                    VALUES ('{$int_id}', '{$loan_id}', '{$client_id}', 
                                                                    '{$collection_id}', '{$repayment_start}', 
                                                                    '{$matured_date}', '1', '{$collection_principal}', 
                                                                    '{$collection_interest}', '0', '0', NULL, '{$gen_date}', 
                                                                    '{$gen_date}')");
                                    if ($update_repayment) {
